...
class IssueUpdateAPIView(generics.UpdateAPIView):
    ...
    def get_object(self) -> Issue:
        ...
        if not (user.is_staff or user == issue.assigned_to):
            logger.warning(
                f"Unauthorized issue status update attempt by user {user.get_user_full_name} on issue {issue.title}"
            )
...
class IssueDeleteAPIView(generics.DestroyAPIView):
    ...
    def get_object(self) -> Issue:
        ...
        user = self.request.user
        if not (user == issue.reported_by):
            logger.warning(
                f"Unauthorized delete attempt by user {user.get_user_full_name} on issue {issue.title}"
            )
