...
class PostDetailAPIView(generics.RetrieveAPIView):
    ...
    def record_post_view(self, post) -> None:
...
class BookmarkPostAPIView(APIView):
    def patch(self, request, slug) -> Response:
        user = request.user
        ...
        return Response(
            {"message": "Post bookmarked succesfully"}, status=status.HTTP_200_OK
        )
...
class UnBookmarkPostAPIView(APIView):
    def patch(self, request, slug) -> Response:
        ...
        if not post.bookmarked_by.filter(id=user.id).exists():
            return Response(
                {
...
class UpvotePostAPIView(APIView):
    def patch(self, request, post_id) -> Response:
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"status": "success", "message": "Post upvoted successfully!"},
...
class DownvotePostAPIView(APIView):
    def patch(self, request, post_id) -> Response:
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"status": "success", "message": "Post downvoted successfully!"},
                status=status.HTTP_200_OK,
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
